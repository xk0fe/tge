...
function  DIGetMouseState(var _x, _y, _z: longint; var _0u, _0d, _0z,
...
begin
  ...
  repeat
    ...
    if (_hr = DIERR_INPUTLOST) or (_hr = DIERR_NOTACQUIRED) then
      begin                    // |--- Added by Gebbiz ---|
        _hr := DIM8.Acquire;
        if not failed(_hr)
    ...
    case _od.dwOfs of
      ...
      DIMOFS_Z : _z := _z + longint(_od.dwData);
...
begin
  ...
  // Set the cooperative level.
  if failed(DIK8.SetCooperativeLevel(_hWnd, DISCL_FOREGROUND or DISCL_NONEXCLUSIVE)) then exit;
...
begin
  ...
  if (_hr = DIERR_INPUTLOST) or (_hr = DIERR_NOTACQUIRED) then
    begin                    // |--- Added by Gebbiz ---|
      _hr := DIK8.Acquire;
      if not failed(_hr)
        then _hr := DIK8.GetDeviceState(SizeOf(DIKeyBuffer), @DIKeyBuffer);
    end;
